...
serve(async (req)=>{
  ...
  try {
    ...
    // Simplified logic for both BPO Transfer and Agent Callback
    ...
    if (existingEntry) {
      ...
      const { data, error } = await supabase.from('daily_deal_flow').update({
        ...
        status: finalStatus,
        call_result: determineCallResultStatus(application_submitted, sent_to_underwriting),
        carrier,
        product_type,
        draft_date,
        monthly_premium,
        ...
        level_or_gi,
        from_callback: call_source === "Agent Callback",
        is_callback: submission_id.startsWith('CBB'),
      ...
      // No entry exists for today - create new entry with CBB prefix if needed
      ...
      const { data, error } = await supabase.from('daily_deal_flow').insert({
        ...
        level_or_gi,
        from_callback: call_source === "Agent Callback",
        is_callback: true // New entries with CBB prefix are always callbacks
      }).select().single();
    ...
    // Note: lead_vendor should be passed in the request body for this to work
    // if (application_submitted === true && leadData?.lead_vendor) {
    //   // Lead vendor update logic would go here if needed
    // }
