...
interface AmericanHomeLifeStateAvailability {
  state_name: string;
  state_code: string;
...
export default function AmericanHomeLifeStateAvailabilityManager() {
  ...
  const [hasAmericanHomeLifeLicense, setHasAmericanHomeLifeLicense] = useState(false);
  ...
  useEffect(() => {
    if (selectedAgentId) {
      checkAmericanHomeLifeLicense(selectedAgentId);
      fetchAmericanHomeLifeStateAvailability(selectedAgentId);
    }
  }, [selectedAgentId]);
  ...
  const checkAmericanHomeLifeLicense = async (agentUserId: string) => {
    ...
    try {
      const { data, error } = await supabase
        ...
        .eq('carriers.carrier_name', 'American Home Life')
        .single();

      if (error && error.code !== 'PGRST116') throw error;
      setHasAmericanHomeLifeLicense(data?.is_licensed || false);
    } catch (error) {
      console.error('Error checking American Home Life license:', error);
      setHasAmericanHomeLifeLicense(false);
    } finally {
      setCheckingLicense(false);
    }
  };

  const fetchAmericanHomeLifeStateAvailability = async (agentUserId: string) => {
    try {
      const { data, error } = await supabase
        .from('american_home_life_agent_state_availability')
      ...
      console.error('Error fetching American Home Life state availability:', error);
  ...
  const enableAmericanHomeLifeLicense = async () => {
    ...
    try {
      const { data: carrierData, error: carrierError } = await supabase
        .from('carriers')
        .select('id')
        .eq('carrier_name', 'American Home Life')
        .single();
      ...
      setHasAmericanHomeLifeLicense(true);
      toast({
        title: 'Success',
        description: 'American Home Life carrier license enabled.',
      });
    } catch (error) {
      console.error('Error enabling American Home Life license:', error);
      toast({
        title: 'Error',
        description: 'Failed to enable American Home Life license.',
        variant: 'destructive'
      });
    } finally {
      setSaving(false);
  ...
  const saveChanges = async () => {
    if (!selectedAgentId || !hasAmericanHomeLifeLicense) {
      toast({
        title: 'Error',
        description: 'Agent must have American Home Life carrier license first.',
        variant: 'destructive'
      });
      return;
    }
    ...
    try {
      ...
      toast({
        title: 'Success',
        description: `American Home Life state availability updated for all ${ALL_STATES.length} states.`,
      });

      await fetchAmericanHomeLifeStateAvailability(selectedAgentId);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            American Home Life State Availability Manager
          </CardTitle>
          <CardDescription>
            Manage agent-specific state availability for American Home Life carrier (requires upline licensing for all 52 states)
          </CardDescription>
        </CardHeader>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-4 p-3 bg-muted rounded-lg">
                ...
                <div className="flex gap-3">
                  ...
                  ) : hasAmericanHomeLifeLicense ? (
                    <Badge variant="default" className="flex items-center gap-1 bg-green-600">
                      <CheckCircle2 className="h-3 w-3" />
                      American Home Life Licensed
                    </Badge>
                  ) : (
                    <Badge variant="destructive" className="flex items-center gap-1">
                      <AlertCircle className="h-3 w-3" />
                      No American Home Life License
                    </Badge>
                  )}
                  <Badge variant="secondary" className="flex items-center gap-1">
                ...
                <Alert className="bg-orange-50 border-orange-200">
                  ...
                  <AlertDescription className="text-orange-800">
                    <div className="flex items-center justify-between">
                      <span>This agent needs an American Home Life carrier license before state availability can be configured.</span>
                      <Button 
                        size="sm" 
                        onClick={enableAmericanHomeLifeLicense}
                        disabled={saving}
                      >
                        {saving ? (
                          <>
                          ...
                          'Enable American Home Life License'
                        )}
                      </Button>
                    </div>
                  </AlertDescription>
              ...
              <Alert className="bg-purple-50 border-purple-200">
                ...
                <AlertDescription className="text-purple-800 text-sm">
                  <strong>Important:</strong> American Home Life uses a separate state availability system. All states require upline license verification. Each agent can have custom YES/NO settings per state.
                </AlertDescription>
              </Alert>
            </div>
          )}
      ...
      {selectedAgentId && hasAmericanHomeLifeLicense && (
        <Card>
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Select states where {selectedAgent?.display_name} can receive American Home Life leads
                </CardDescription>
              </div>
              ...
              >
                {saving ? (
                  ...
                  <>
                    ...
                    Save American Home Life State Availability
                  </>
                )}
              </Button>
            </div>
