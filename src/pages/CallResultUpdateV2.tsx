...
const CallResultUpdateV2 = () => {
  ...
  const [isCallFormOpen, setIsCallFormOpen] = useState(true);
  ...
  return (
    <div className="min-h-screen bg-background p-4">
      <div className="max-w-7xl mx-auto space-y-6">
        <div className="flex items-center gap-4 w-full">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-3xl font-bold whitespace-nowrap">
              {fromCallback ? "Update Callback Result" : "Update Call Result"} V2
            </h1>
        ...
        {showVerificationPanel && verificationSessionId ? (
          <>
            {/* Main Content - New Layout: Left (Verification) | Right Top (Toolkit) & Right Bottom (Call Form) */}
            <div className="grid grid-cols-12 gap-4">
              {/* Left Column - Verification Panel (4 cols) */}
              <div className="col-span-12 lg:col-span-4">
                <Collapsible open={isVerificationOpen} onOpenChange={setIsVerificationOpen}>
                  <div className="border rounded-lg overflow-hidden">
                    <CollapsibleTrigger asChild>
                      <div className="bg-muted p-3 cursor-pointer flex items-center justify-between hover:bg-muted/80">
                        <span className="font-semibold">Verification Panel</span>
                        {isVerificationOpen ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent>
                      <VerificationPanel 
              ...
              {/* Right Column - Toolkit (Top) & Call Form (Bottom) (8 cols) */}
              <div className="col-span-12 lg:col-span-8 space-y-4">
                {/* Health Kit Tool - Top */}
                <Collapsible open={isToolkitOpen} onOpenChange={setIsToolkitOpen}>
                  <div className="border rounded-lg overflow-hidden">
                    <CollapsibleTrigger asChild>
                      <div className="bg-muted p-3 cursor-pointer flex items-center justify-between hover:bg-muted/80">
                        <span className="font-semibold">Health Kit - Rate Quote Tool</span>
                        {isToolkitOpen ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent>
                      <div className="p-3">
                        <div className="border-2 border-blue-300 rounded-lg overflow-hidden bg-white" style={{ height: '400px' }}>
                          <iframe
                            style={{ border: 'none', height: '100%', width: '100%' }}
                            src="https://insurancetoolkits.com/login"
                            title="Health Kit Login"
                            className="health-kit-frame"
                            id="healthKitIframePageV2"
                          />
                        </div>
                        <div className="flex gap-2 mt-2">
                          <Button
                            ...
                            size="sm"
                            onClick={() => {
                              const iframe = document.getElementById('healthKitIframePageV2') as HTMLIFrameElement;
                ...
                {/* Call Result Form - Bottom */}
                <Collapsible open={isCallFormOpen} onOpenChange={setIsCallFormOpen}>
                  <div className="border rounded-lg overflow-hidden">
                    <CollapsibleTrigger asChild>
                      <div className="bg-muted p-3 cursor-pointer flex items-center justify-between hover:bg-muted/80">
                        <span className="font-semibold">Call Result Form</span>
                        {isCallFormOpen ? <ChevronDown className="h-5 w-5" /> : <ChevronRight className="h-5 w-5" />}
                      </div>
                    </CollapsibleTrigger>
                    <CollapsibleContent>
                      <CallResultForm 
...
export default CallResultUpdateV2;
